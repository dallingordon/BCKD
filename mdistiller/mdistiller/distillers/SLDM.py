...
class SLDM(Distiller):
    """Shuffled Label Difference (Cosine Distance) dg :)"""

    def __init__(self, student, teacher, cfg):
        super(SLDM, self).__init__(student, teacher)
        
        self.sld_loss_weight = cfg.SLDM.X_WEIGHT
        self.sld_loss_weight_intrinsic = cfg.SLDM.I_WEIGHT
        self.M = cfg.SLDM.M
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        # losses
        loss_sld = self.sld_loss_weight * sldcd_loss(logits_student,logits_teacher*self.M)
        loss_sld_intrinsic = self.sld_loss_weight_intrinsic * sldintrinsic_loss(logits_student,logits_teacher*self.M)
