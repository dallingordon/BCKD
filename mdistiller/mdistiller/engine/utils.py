...
##added this myself for testing.
def validate_debug(val_loader, distiller, results_file_path):
    batch_time, losses, top1, top5 = [AverageMeter() for _ in range(4)]
    criterion = nn.CrossEntropyLoss()
    num_iter = len(val_loader)
    pbar = tqdm(range(num_iter))

    # Open a file to write the predicted and actual labels
    with open(results_file_path, 'w') as f:
        f.write("Predicted,Actual\n")  # Header for the CSV file

    distiller.eval()
    with torch.no_grad(), open(results_file_path, 'a') as f:
        start_time = time.time()
        for idx, (image, target) in enumerate(val_loader):
            image = image.float()
            image = image.cuda(non_blocking=True)
            ...
            predicted = predicted.t().cpu().numpy().flatten()
            actual = target.cpu().numpy()

            # Write each pair of predicted and actual labels to file
